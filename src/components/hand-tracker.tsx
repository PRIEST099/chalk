...
export function HandTracker({ active, teacherName, onTeacherNameChange, status, onPoint, onPinch, onSwipeLeft, onError, onEnabledChange, onHandLost, onDebug }: Props) {
  ...
  return <section className="absolute bottom-5 right-5 z-20 w-[224px] overflow-hidden rounded-2xl border border-white/20 bg-[#101a1c] shadow-2xl shadow-black/40">
    <div className="relative aspect-[16/10] bg-[#1d2d2e]">
      ...
      {!enabled && <div className="absolute inset-0 grid place-items-center bg-[radial-gradient(circle_at_30%_20%,#39514d,transparent_55%),#172326]"><div className="grid size-16 place-items-center rounded-full border border-[#e6a75b]/35 bg-[#e6a75b]/15 text-xl font-semibold tracking-wide text-[#f3c887]">{initials}</div><span className="absolute bottom-4 text-xs text-[#a8b3ad]">{starting ? "Starting camera…" : "Camera off"}</span></div>}
      <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-gradient-to-t from-black/80 to-transparent px-3 pb-2.5 pt-7"><div className="min-w-0"><input aria-label="Presenter name" value={teacherName} onChange={(event) => onTeacherNameChange(event.target.value)} className="w-full bg-transparent text-sm font-semibold text-[#f5f0df] outline-none placeholder:text-[#f5f0df]/65" placeholder="Teacher" /><p className="text-[10px] uppercase tracking-[0.16em] text-[#c4cfca]">Presenter</p></div><span title={status === "Idle" ? "Microphone stopped" : `${status} microphone`} className={`ml-2 grid size-7 shrink-0 place-items-center rounded-full border border-white/20 ${micTone}`}><MicGlyph /></span></div>
    </div>
  </section>;
}
